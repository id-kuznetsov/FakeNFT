...
1. **Показ окна выбора сортировки** 
   - Создание модального экрана с вариантами сортировки (est: 1 час; fact: x часов).
### Верстка экрана выбора способа оплаты
2. **Заголовок с кнопкой возврата**  
   - Создание кастомного `title` и кнопки "Назад" (est: 1 час; fact: x часов).
3. **Коллекция доступных способов оплаты**  
   - Создание `UICollectionView` для отображения способов оплаты (est: 3 часа; fact: x часов).  
   - Кастомизация ячеек коллекции (est: 1 час; fact: x часов).
4. **Пользовательское соглашение**  
   - Добавление текстового элемента со ссылкой на пользовательское соглашение (est: 1 час; fact: x часов).
5. **Веб-вью для показа пользовательского соглашения**  
   - Реализация модального экрана с веб-вью для просмотра соглашения (est: 2 часа; fact: x часов).
6. **Кнопка оплаты**  
   - Создание кнопки "Оплатить" (est: 1 час; fact: x часов).
#### Логика
6.  **Логика сортировки**
    - Реализация различных методов сортировки (по цене, рейтингу, по названию) (est: 2 часа; fact: x часов).
    - Сохранение выбранного метода сортировки (est: 1 час; fact: x часов).
7.  **Загрузка списка способов оплаты**  
  	 - Выполнение запроса для получения списка доступных способов оплаты (est: 3 часа; fact: x часов).
